import { IconCpu, IconFileText, IconZap, IconNetwork } from './Icons'
/**
 * RiskScoreBreakdown — visual breakdown of risk score components
 */

const _COMPONENTS = [
  { key: 'anomaly_score', label: 'Anomaly Score', description: 'ML anomaly detection confidence', icon: null },
  { key: 'rule_score',    label: 'Rule Match',     description: 'Signature & rule-based detection', icon: null },
]
...
function SeverityScore({ severity }) {
  ...
  return <ScoreBar value={value} label="Severity Weight" description="Classification severity multiplier" icon={<IconZap className="w-4 h-4 text-amber-400" />} />
}

function CorrelationScore({ eventCount }) {
  const value = Math.min(1, eventCount / 5)
  return <ScoreBar value={value} label="Correlation Depth" description={`${eventCount} correlated event${eventCount !== 1 ? 's' : ''} linked`} icon={<IconNetwork className="w-4 h-4 text-purple-400" />} />
}
  ...
  return (
    <div className="glass-card p-5 space-y-5">
      ...
      <div className="border-t border-white/5 pt-4 space-y-4">
        <ScoreBar value={avgAnomaly} label="Anomaly Score" description="ML anomaly detection confidence (avg across events)" icon={<IconCpu className="w-4 h-4 text-cyan-400" />} />
        <ScoreBar value={avgRule}    label="Rule Match Score" description="Signature & rule-based detection (avg across events)" icon={<IconFileText className="w-4 h-4 text-indigo-400" />} />
