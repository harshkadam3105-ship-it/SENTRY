...
class RuleEngine:
    ...
    def brute_force_rule(self, event: Dict[str, Any]):
        ...
        cfg = (self.config or RULE_CONFIG)["brute_force"]
    ...
    def failed_then_success_rule(self, event: Dict[str, Any]):
        features = event.get("features", {})
        cfg = (self.config or RULE_CONFIG)["failed_then_success"]

        failed_logins = features.get("failed_login_count", 0)
        successful_logins = features.get("success_count", 0)

        if (
            failed_logins >= cfg["failed_login_threshold"]
            and successful_logins >= cfg["success_login_threshold"]
        ):
            return {
                "score": cfg["score"],
    ...
    def suspicious_process_rule(self, event: Dict[str, Any]):
        features = event.get("features", {})
        cfg = (self.config or RULE_CONFIG)["suspicious_process"]
    ...
    def unusual_outbound_rule(self, event: Dict[str, Any]):
        ...
        cfg = (self.config or RULE_CONFIG)["unusual_outbound"]
    ...
    def api_abuse_rule(self, event: Dict[str, Any]):
        ...
        cfg = (self.config or RULE_CONFIG)["api_abuse"]
